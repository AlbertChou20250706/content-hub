...
- 候選池：台股個股／美股個股各 15 檔、台股 ETF／美股 ETF 各 10 檔知名龍頭與主流 ETF（非全市場掃描，免費 API＋GitHub Actions 排程架構下不現實），程式依量化公式排序取前 10 名
- 候選池組成是使用者要求擴大後的結果——一開始只做前 3 名、候選池 6～8 檔，使用者實測看過第一批結果後，認為只顯示 3 檔太少，才請求擴大到前 10 名，候選池也同步擴大到 10～15 檔（否則「前10名」會等於候選池全部，篩選失去意義）
...
- 排序結果由程式決定，Claude 只負責引用程式算好的數字說明「為什麼夠格」，不能重新排序、不能自己推薦排序外的標的；每檔的入選理由改成一行一句話（不是一整段），並依代號逐一比對貼回該檔的卡片列，避免 10 檔的完整段落文字讓卡片過長難讀
