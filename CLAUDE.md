...
- 一樣受本檔案「內容規範」約束（見下方，不放二進位檔的限制仍適用）
...
## 內容規範：Markdown 與程式碼皆可放，二進位檔仍另外處理

> ⚠️ 2026-09-17 規範異動：原本「只放 Markdown、不放程式碼」的限制已放寬，這個 repo 現在**可以放程式碼**（原始碼、腳本、設定檔等工程檔案），不再侷限於 Markdown 內容。**注意：`content-hub` 是公開（public）repo**，所以是否把程式碼放進來，要先過下面的「未公開內容」判斷。

- 這個 repo **可放 Markdown 內容與程式碼**
- **不放圖片／影片等二進位檔**（截圖、影片檔請放在其他地方，這裡只放連結）——這條仍然有效，沒有變動
- 例外：主題資料夾內的 `notebooklm_sources/` 可放 `.html` 文字來源文件（用途見上方「新增主題的標準流程」第 5 點）
- 目的：在保留內容飛輪定位的同時，讓 repo 可以承載相關工程素材

### 新自動化／程式碼要放哪裡：先判斷是否涉及「未公開內容」

規劃新的自動化主題或工程素材時，先做這個判斷：

1. **涉及未公開內容**（例如：尚未發表的商業策略、變現細節、未上架產品的內部邏輯、個人財務／持股資訊等敏感資料）→ **維持獨立 repo 模式**，且該 repo 預設為 **private**，不進 `content-hub`
2. **不涉及未公開內容**（單純是已發布或準備公開的技術示範、工具原始碼等）→ 可以**直接把程式碼寫進 `content-hub` 對應主題資料夾**（`topics/YYYY-MM-DD_主題slug/` 底下），跟 README／metadata 放在一起

**保密優先原則**：往後只要涉及撰寫程式碼或建立自動化，預設先以保密為主（傾向獨立 private repo，而不是直接寫進公開的 `content-hub`）。如果對於「這個該公開還是該保密」有疑慮或不確定，**先告知 Albert，由他決策，不要自行判斷放行**。
