...
- 一樣受本檔案「可放程式碼與 Markdown」的總規範約束（見下方「內容規範」）
...
## 內容規範：可放程式碼與 Markdown，保持精簡

- 這個 repo 可以放 **Markdown 內容**，也可以放**程式碼**（原始碼、腳本、CI 設定等工程檔案）
- **不放圖片／影片等二進位檔**（截圖、影片檔請放在其他地方，這裡只放連結）
- 主題資料夾內的 `notebooklm_sources/` 可放 `.html` 文字來源文件（用途見上方「新增主題的標準流程」第 5 點）
- 目的是保持 repo 精簡、易於檢索與長期維護

## 新自動化的存放位置：以保密為預設，使用者最終決定

- **預設保密**：規劃或撰寫新的程式碼／自動化時，預設先假設涉及未公開內容，不要主動把程式碼直接寫進 content-hub 或以其他方式公開。如果認為某個自動化其實不涉及機密、可以直接放進 content-hub，要先告知使用者說明理由，由使用者決策，不能自行判斷後就動手。
- **是否涉及「未公開內容」由使用者判斷，不是 Claude 自行認定**：
  - 涉及未公開內容 → 維持獨立 repo 模式（工程程式碼與 content-hub 分離，例如目前的 `ai-stock-weekly-report-bot`、`stock-committee-bot`、`daily-tool-digest` 等）
  - 不涉及未公開內容（經使用者確認）→ 可以直接把程式碼寫進 content-hub 對應主題資料夾裡，不用另開獨立 repo
