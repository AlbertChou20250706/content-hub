...
## 內容規範：Markdown 與程式碼皆可，不放二進位檔

- 這個 repo 可以放 **Markdown 內容**，也可以放 **程式碼**（原始碼、腳本、CI 設定等工程檔案）——不再侷限只放 Markdown
- **不放圖片／影片等二進位檔**（截圖、影片檔請放在其他地方，這裡只放連結）
- 例外：主題資料夾內的 `notebooklm_sources/` 可放 `.html` 文字來源文件（用途見上方「新增主題的標準流程」第 5 點）
- 目的是保持 repo 易於檢索與長期維護

## 新自動化的放置原則（保密優先）

- **content-hub 是 public repo**——規劃新的自動化流程／工具程式碼時，預設先以保密／私有為主，不要預設直接寫進這個 repo
- 若對某個自動化該不該保密有疑慮、不確定，**主動告知 Albert，由他決策**，不要自行認定
- 放置位置由 Albert 依「是否涉及未公開內容」判斷：
  - **涉及未公開內容** → 維持獨立 repo（private）模式，比照 `daily-tool-digest`、`yt-daily-report` 的做法
  - **不涉及未公開內容** → 可以直接把程式碼寫進 `content-hub` 對應主題資料夾
