...
## 內容規範：Markdown 與程式碼並存，保持精簡

- 這個 repo 以 **Markdown 內容為主**，同時允許存放**程式碼／腳本／CI 設定**（例如自動化流程用的腳本、`.github/workflows/` 下的 GitHub Actions），不再侷限於單一用途
- **不放圖片／影片等二進位檔**（截圖、影片檔請放在其他地方，這裡只放連結），避免 repo 因媒體檔案膨脹
- 例外：主題資料夾內的 `notebooklm_sources/` 可放 `.html` 文字來源文件，用途見上方「新增主題的標準流程」第 5 點
- 目的：讓內容飛輪的文字紀錄跟支撐它的自動化程式碼可以放在同一個 repo 管理，同時仍保持精簡、易於檢索

## 新自動化主題的程式碼該放哪裡：先判斷是否涉及未公開內容

規劃新的自動化主題時，先判斷它是否涉及「未公開內容」（例如未發布的商業策略、尚未公開的產品規劃、內部限定資訊等）：

- **涉及未公開內容** → 維持獨立 repo，不放進 `content-hub`
- **不涉及未公開內容** → 程式碼可以直接寫進 `content-hub` 對應主題資料夾（`topics/<主題>/` 或 `daily-tools/<日期>_<slug>/`）底下

這個判斷由 Albert 主動做，Claude 不用自己猜——每次規劃新自動化主題前，先跟 Albert 確認要放哪裡。

**保密優先原則**：寫程式碼或自動化流程時，預設以保密／低曝光為優先（例如 commit 訊息、程式碼註解、log 輸出裡不外露不必要的內部細節、憑證或未公開資訊）。若對某段內容是否該公開有疑慮，**不要自行決定發布**，先回報給 Albert，由他決策。
